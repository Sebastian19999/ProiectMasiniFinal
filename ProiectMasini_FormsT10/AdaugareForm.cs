...
namespace ProiectMasini_FormsT10
{
    public partial class AdaugareForm : Form
    {
        ...
        public string stocareLocatieImagine { get; set; }
        public string salvareUser { get; set; }

        public AdaugareForm(string userTxt)
        {
            InitializeComponent();
            salvareUser = userTxt;
        ...
        private CodEroare Validare(string firma, string model,string anFabricatie,
                    string numeVanzator, string dataTranzactie, string pret,string tip)
        {
            ...
            else if (anFabricatie == string.Empty)
            {
                try
                {
                    int anFTry = Int32.Parse(anFTxt.Text);
                    if (anFTry < 1900)
        ...
        private void adaugaBtn_Click(object sender, EventArgs e)
        {
            ...
            CodEroare valideaza = Validare(firmaTxt.Text, modelTxt.Text,
                //culoareTxt.Text,
                anFTxt.Text, numeVanzatorTxt.Text, dataTxt.Text, pretTxt.Text,
                //, optiuniTxt.Text
                tipCmbBox.Text);
            ...
            if (valideaza != CodEroare.CORECT)
            {
                switch (valideaza)
                {
                    ...
                    case CodEroare.TIP_INCORECT:
                        if (tipCmbBox.Text == string.Empty)
                            tipMasinaLbl.ForeColor = Color.Red;
                        break;
                }
            ...
            else
            {
                ...
                Imagine img = new Imagine(stocareLocatieImagine);
                ManagerImg.ManagerImagini.addImagine(img);
                List<Imagine> imagini = new List<Imagine>(ManagerImg.ManagerImagini.getImagini());
                ...
                anunt = new Anunt(numeVanzatorTxt.Text.ToString()
                , firmaTxt.Text.ToString(), modelTxt.Text.ToString(), Convert.ToInt32(anFTxt.Text.ToString()),
                "rosu", optiuniMasinaForm, dataTxt.Text.ToString(), Convert.ToDouble(pretTxt.Text.ToString()), tip, DateTime.Now,salvareUser,stocareLocatieImagine,0);


                anunt.CuloareMasina = GetCuloareSelectata();
                anunt.identificaCuloare();

                

                anunt.Optiuni = new ArrayList();
                anunt.Optiuni.AddRange(optiuniSelectate);





                ManagerAnunturi.addAnunt(anunt);
        ...
        private void addImgBtn_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    locatieImagine = dialog.FileName;
                    adaugaImaginePict.ImageLocation = locatieImagine;
                    stocareLocatieImagine = locatieImagine;
                }
            }
            catch (Exception)
            {
                MessageBox.Show("Eroare la adaugarea imaginii", "Eroare", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
            }
        }
